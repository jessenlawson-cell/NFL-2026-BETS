...
def test_challenger_contract_uses_direct_football_projection() -> None:
    ...
    candidate = ChallengerCandidate(
        ...
        total_residuals=mapper,
        spread_calibrator=calibrator,
        total_calibrator=calibrator,
        spread_probability_margin=0.03,
        total_probability_margin=0.04,
    ...
    assert result["final_projection"] == pytest.approx(4.0)
    assert result["raw_adjustment"] == pytest.approx(1.0)
    assert result["adjustment_weight"] == pytest.approx(1.0)
    ...
    assert quantitative_drivers(candidate, "spreads", np.asarray([[2.0]]), top=1)[0][
        "feature"
    ] == "x"
...
def test_power_rankings_use_the_frozen_projection_on_a_neutral_matchup() -> None:
    ...
    calibrator = ProbabilityCalibrator().fit(
        np.asarray([0.4, 0.6] * 15), np.asarray([0, 1] * 15)
    )
    candidate = ChallengerCandidate(
        version="challenger-test",
        feature_columns=("st_epa_difference",),
        spread_model=model,
        total_model=model,
        spread_residuals=mapper,
        total_residuals=mapper,
        spread_calibrator=calibrator,
        total_calibrator=calibrator,
        spread_probability_margin=0.0,
        total_probability_margin=0.0,
        selected_half_life=2.0,
        selected_prior_strength=1.0,
        training_cutoff="2026-W02",
        prospective_start_week=3,
    ...
    for week in (2, 3):
        ...
        for is_home, value in ((True, 2.0), (False, 1.0)):
            ...
            row.update(
                {
                    metric: value
                    for metric in (*CHALLENGER_METRICS, *OPPONENT_ADJUSTED_METRICS)
                }
            )
            team_rows.append(row)
    pl.DataFrame(games).write_csv(settings.root / "games.csv")

    result = assemble_challenger_matrix(
        pl.DataFrame(team_rows), settings=settings, through_week=2
    )
...
def test_comparator_reports_missing_matched_evidence_without_inventing_rows(
    ...
    settings = Settings.for_root(tmp_path)
    report = compare_challenger(through_week=3, settings=settings)
